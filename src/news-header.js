...
class NewsHeader extends PolymerElement {
  static get template() {
    ...
    <style>
      ...
      :host([page=list]) .sticky-nav news-share,
      :host([page=list]) .sticky-nav news-share {
        display: none;
      }
      ...
      @media (max-width: 767px) {
        ...
        header .logo {
          /*margin-right: 40px; not needed with the city logo */
        }
        
        header .homeLogo {
          background: url('https://static.auburnalabama.org/media/citylogos/citylogo_invert-notext.png') center center;
      ...
      @media (min-width: 768px) {
        ...
        header .logo {
          margin-left: 72px; /*needed because the city logo */
        }
        header .homeLogo {
          background: url('https://static.auburnalabama.org/media/citylogos/logo-64x64.png') center center;
          background-repeat: no-repeat;
          display: inline-block;
          height: 72px;
          float: right;
      ...
      @media only print {
        paper-icon-button, iron-selector, .menu-list {
          display: none;
        }
      }
    </style>
...
`;
  ...
  _setShare(page) {
    if (page == "article" || this.page == 'article') {
      this.set('shareUrl', window.location.href);
      try {
        this.$.psb.$.shareMenu.set('horizontalAlign', 'right');
      } catch (e) {//hack but it's being lazy loaded so...
        setTimeout(this._setShare.bind(this), 500);
      }
    } 
  }
